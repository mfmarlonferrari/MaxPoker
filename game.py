...
class Game(object):
    ...
    def addPlayer(self, name):
        player = None
        if not self.started or name in self.waitingList:
            player = Player(self, name, self.stackSize)
            self.players.append(player)
        else:
            self.waitingList.append(name)
            print name + ' fica para a proxima rodada'
        return player
    ...
    #funcao que insere os jogadores na lista de espera
    def insertWaitingPlayers(self):
        if self.waitingList:
                for i in range(len(self.waitingList)):
                    self.addPlayer(self.waitingList[i])
